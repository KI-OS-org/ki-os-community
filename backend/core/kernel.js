...
class Kernel {
    async executeRequest(requestPayload, context = {}) {
        ...
        // Smart Router Pre-Hook — Privacy + Task Check → Loki wenn lokal möglich
        try {
            const smartRouter = require("../services/routing/smart-router.service");
            const smartDecision = await smartRouter.route({ text: jobReq.input_text, context: jobReq.intent });
            if (smartDecision.provider === "local") {
                const lokiBridge = require("../services/routing/loki-bridge.service");
                const lokiResult = await lokiBridge.generate({
                    prompt: jobReq.input_text,
                    preferNode: smartDecision.model === "loki-pro" ? "loki-pro" : "loki-mini"
                });
                if (!lokiResult.error) {
                    runtimeStore.transitionRun(runId, "COMPLETED", { stepName: "loki_local", worker: smartDecision.model, model: smartDecision.model });
                    persistRoutingOutcome({ routing: { provider: "local", model: lokiResult.model } }, { success: true, latencyMs: lokiResult.latencyMs, trustScore: 1, outcomeCoverage: 1, runId });
                    return {
                        success: true, runId, text: lokiResult.text,
                        routing: { provider: "local", model: lokiResult.model, node: lokiResult.node },
                        meta: { local: true, privacySensitive: smartDecision.privacySensitive, latencyMs: lokiResult.latencyMs }
                    };
                }
            } else if (smartDecision.provider && smartDecision.model) {
                routing.routing.provider = smartDecision.provider;
                routing.routing.model = smartDecision.model;
                if (routing.workerTask) {
                    routing.workerTask.provider = smartDecision.provider;
                    routing.workerTask.model = smartDecision.model;
                }
            }
        } catch (smartErr) {
            logger.warn("smart_router.fallback", { message: smartErr.message });
        }
