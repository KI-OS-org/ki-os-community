/**
 * KI-OS Orbit Control — UI Runtime Store
 * Copyright (c) Ingo Schaffer
 *
 * Zweck:
 * File-basierter Store für Runtime-, Trace-, Memory- und File-Daten.
 * Dient als gemeinsame echte lokale Quelle für L2 LIVE-light APIs.
 *
 * Status:
...
import { createUiRuntimeSeed } from "./ui-runtime-seed";
...
export function resetUiRuntimeState() {
  return saveUiRuntimeState(createUiRuntimeSeed());
}

export function updateUiRuntimeState(updater) {
  return saveUiRuntimeState(updater(cloneValue(loadUiRuntimeState())));
}
