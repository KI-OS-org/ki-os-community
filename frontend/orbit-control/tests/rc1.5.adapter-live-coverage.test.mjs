...
test("RC1.5 live adapters use orbitFetch", async () => {
...
test("RC1.5 capability badge exists", async () => {
  assert.equal(fs.existsSync(path.join(root, "frontend/orbit-control/components/shared/capability-badge.tsx")), true);
});

test("RC1.5 install scripts exist", async () => {
  assert.equal(fs.existsSync(path.join(root, "install-local.cmd")), true);
  assert.equal(fs.existsSync(path.join(root, "install-local.sh")), true);
});
