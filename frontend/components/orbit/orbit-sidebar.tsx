  ...
  useEffect(() => {
    ...
    async function ping() {
      try {
        ...
        if (data?.os_level) setVersion(data.os_level);
        if (data?.edition) setEdition(data.edition);
        setStatus(data ? "online" : "offline");
      } catch {
        setStatus("offline");
      }
    }
    ...
    <div className="mt-auto rounded-[20px] border border-white/10 bg-black/20 px-3 py-2.5 space-y-1.5">
      ...
      <div className="border-t border-white/8 pt-2">
        <div className="text-[10px] font-mono text-[var(--accent)] tracking-wide flex items-center gap-0.5">
          ...
          <span>OS {version ? `v${version}` : "v1.12.0"}  ✦</span>
        </div>
        {edition && (
          <span
            className={cn(
              "inline-block mt-0.5 px-1.5 py-0.5 rounded text-[9px] font-bold tracking-widest uppercase",
              edition === "enterprise"
